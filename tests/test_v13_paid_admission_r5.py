...
VERIFIER_PATH = (
    REPOSITORY
    / "benchmark_records/reproduction-v13-paid-paired-scoring-r5-20260716/verify_dynamic_admission.py"
)
...
class DynamicAdmissionMutationTests(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        shutil.copy2(
            REPOSITORY
            / "benchmark_records/reproduction-v13-paid-paired-scoring-r5-20260716/verify_evaluator_ledger.py",
            self.ledger_verifier,
        )
        self.telemetry_capture = self.revision / "capture_provider_telemetry.py"
        shutil.copy2(
            REPOSITORY
            / "benchmark_records/reproduction-v13-paid-paired-scoring-r5-20260716/capture_provider_telemetry.py",
            self.telemetry_capture,
        )
        self.fx_capture = self.revision / "capture_ecb_fx.py"
        shutil.copy2(
            REPOSITORY
            / "benchmark_records/reproduction-v13-paid-paired-scoring-r5-20260716/capture_ecb_fx.py",
            self.fx_capture,
        )
        self.credential_launcher = self.revision / "launch_with_openrouter_key.sh"
        shutil.copy2(
            REPOSITORY
            / "benchmark_records/reproduction-v13-paid-paired-scoring-r5-20260716/launch_with_openrouter_key.sh",
            self.credential_launcher,
        )
        self.paid_wrapper = self.revision / "run_paid_variant_hardened.sh"
        shutil.copy2(
            REPOSITORY
            / "benchmark_records/reproduction-v13-paid-paired-scoring-r5-20260716/run_paid_variant_hardened.sh",
            self.paid_wrapper,
        )
        self.harness_guard = self.revision / "run_harness_guarded.py"
        shutil.copy2(
            REPOSITORY
            / "benchmark_records/reproduction-v13-paid-paired-scoring-r5-20260716/run_harness_guarded.py",
            self.harness_guard,
        )
        runtime_members = {
            "proxy_config": "config_runtime.py",
            "proxy_runtime": "openrouter_proxy_runtime.py",
            "proxy_guard": "run_openrouter_proxy_guarded.py",
            "arm_gate": "arm_gate_runtime.py",
            "arm_gate_evaluation_auditor": "evaluation_audit_runtime.py",
            "arm_gate_guard": "run_arm_gate_guarded.py",
        }
        bundle = (
            REPOSITORY
            / "benchmark_records/reproduction-v13-paid-paired-scoring-r5-20260716"
        )
        for attribute, filename in runtime_members.items():
            destination = self.revision / filename
        ...
        for label, project, initial in (
            ...
            self.variants[label] = {
                ...
                "question_ids_sha256": _sha(self.question_ids),
                "question_ids_path": str(self.question_ids.relative_to(self.root)),
        ...
        self.r4_cost_record = self.root / "budgets/r4-cost-audit.json"
        _write_json(self.r4_cost_record, {"schema": "immutable-r4-cost"})
        ...
        _write_json(
            self.declaration,
            {
                ...
                "prior_immutable_costs": [
                    ...
                    {
                        "source_id": "r4-v7-terminal-ledger-identity-audit",
                        "record_path": "r4-cost-audit.json",
                        "record_sha256": _sha(self.r4_cost_record),
                        "cost_usd": "0.92196287",
                    }
                ],
    ...
    def _write_provider(
        ...
        *,
        ...
        usage: str = "122.284488918",
        remaining: str = "127.715511082",
    ...
    def _write_requirements(self) -> None:
        document = {
            "schema_version": "narratordb.v13-paid-dynamic-admission-requirements.v4",
            "predecessor": {
            ...
            "revision": {
                ...
                "proxy_config_path": str(self.proxy_config.relative_to(self.root)),
                "proxy_config_sha256": _sha(self.proxy_config),
                "proxy_runtime_path": str(self.proxy_runtime.relative_to(self.root)),
                "proxy_runtime_sha256": _sha(self.proxy_runtime),
                "proxy_guard_path": str(self.proxy_guard.relative_to(self.root)),
                "proxy_guard_sha256": _sha(self.proxy_guard),
                "arm_gate_path": str(self.arm_gate.relative_to(self.root)),
                "arm_gate_sha256": _sha(self.arm_gate),
                "arm_gate_evaluation_auditor_path": str(
                    self.arm_gate_evaluation_auditor.relative_to(self.root)
                ),
                "arm_gate_evaluation_auditor_sha256": _sha(
                    self.arm_gate_evaluation_auditor
                ),
                "arm_gate_guard_path": str(self.arm_gate_guard.relative_to(self.root)),
                "arm_gate_guard_sha256": _sha(self.arm_gate_guard),
            },
            "campaign": {
                ...
                "baseline_observed_usd": "109.23207164191",
                "provider_cap_usd": "250",
                "governance_ceiling_eur": "300",
            },
            "provider": {
                ...
                "currency": "USD",
                "provider_limit_usd": "250",
                "historical_usage_floor_usd": "122.284488918",
            ...
            "phases": {
                ...
                "before-v13": {
                    ...
                    "prior_arm_gate_path": "attempt1/v7-control/evaluation/arm-gate.json",
                },
            },
            "finalization": {
                ...
                "v7_arm_gate_path": "attempt1/v7-control/evaluation/arm-gate.json",
                "v13_arm_gate_path": "attempt1/v13-first/evaluation/arm-gate.json",
    ...
    def _prepare_finalization(self) -> tuple[Path, Path, Path]:
        ...
        def proxy_summary(cost: float, *, initial: bool = False) -> dict:
            return {
                "calls": 0 if initial else 2,
                "errors": 0,
                "malformed_responses": 0,
                "cost_usd": 0 if initial else cost,
                "prompt_tokens": 0 if initial else 20,
                "cached_tokens": 0,
                "completion_tokens": 0 if initial else 4,
                "reasoning_tokens": 0,
        ...
        def complete_arm(label: str, scores: tuple[int, int], cost: float) -> None:
            ...
            evaluation_root = self.root / variant["run_root"] / "evaluation"
            ledger = self.root / variant["ledger_path"]
            evaluator_log = evaluation_root / "evaluate.log"
            evaluator_log.write_text("evaluation complete\n", encoding="utf-8")
            evaluator_log.chmod(0o444)
            ledger.chmod(0o444)

            providers = ["DeepInfra", "StreamLake", "GMICloud", "Baidu", "AtlasCloud"]
            proxy_log = evaluation_root / "proxy.log"
            startup = {
                "ok": True,
                "url": "http://127.0.0.1:8890/v1",
                "provider_only": None,
                "provider_allow": providers,
                "reasoning_effort": "high",
                "usage_log": str(ledger.resolve()),
                "max_cost_usd": 2.5,
                "request_reservation_usd": 0.05,
                "budget_safety_reserve_usd": 0.01,
            ...
            proxy_log.write_text(
                ...
                + "\n"
                + json.dumps(
                    {"stopped": True, "usage": proxy_summary(cost)}, sort_keys=True
                )
                + "\n",
                encoding="utf-8",
            )
            proxy_log.chmod(0o444)
            health = evaluation_root / "proxy-health-before.json"
            _write_json(
                health,
                {
                    "ok": True,
                    ...
                    "provider_allow": providers,
                    "reasoning_effort": "high",
                    "usage": proxy_summary(0, initial=True),
                },
            )
            health.chmod(0o444)

            gate = evaluation_root / "arm-gate.json"
            command = [
                os.sys.executable,
                "-I",
                "-S",
                "-B",
                str(self.arm_gate_guard),
                "--evaluated-directory",
                str(evaluated),
                "--frozen-directory",
            ...
            command.extend(("--max-cost-usd", "2.50", "--output", str(gate)))
            subprocess.run(
                command,
                cwd=self.root,
            ...
            status = evaluation_root / "attempt-status.json"
            _write_json(
                status,
                {
                    "evaluator_status": "0",
        ...
        for label, (answerer_cost, judge_cost) in costs.items():
            ...
            complete_arm(
                label,
                (36, 38) if label == "v7-control" else (37, 39),
                answerer_cost + judge_cost,
            )
            if label == "v7-control":
                ...
                _write_json(
                    between / "provider.json",
                    {
                        ...
                        "currency": "USD",
                        "provider_limit_usd": "250",
                        "provider_usage_usd": "122.684488918",
                        "provider_remaining_usd": "127.315511082",
                        "capture_tool_sha256": _sha(self.telemetry_capture),
                        "credential_recorded": False,
                        "key_label_recorded": False,
                        "account_identifier_recorded": False,
        ...
        _write_json(
            final_provider,
            {
                ...
                "currency": "USD",
                "provider_limit_usd": "250",
                "provider_usage_usd": "123.184488918",
                "provider_remaining_usd": "126.815511082",
                "capture_tool_sha256": _sha(self.telemetry_capture),
                "credential_recorded": False,
                "key_label_recorded": False,
                "account_identifier_recorded": False,
    ...
    def test_valid_closed_world_bundle_passes(self) -> None:
        ...
        self.assertEqual(report["campaign_projected_usd"], "114.23207164191")
        self.assertEqual(report["projected_eur_ceil_cent"], "122.76")
    ...
    def test_valid_post_pair_reconciliation_authorizes_score_release(self) -> None:
        ...
        self.assertEqual(report["campaign_observed_usd"], "110.13207164191")
        self.assertEqual(report["provider_usage_usd"], "123.184488918")
    ...
    def test_post_pair_provider_delta_mutation_blocks_score_release(self) -> None:
        ...
        with self.assertRaisesRegex(admission.AdmissionError, "not monotonic"):
    ...
    def test_preserved_phase_evidence_mutation_blocks_score_release(self) -> None:
        ...
        document["provider_usage_usd"] = "122.684488919"
        document["provider_remaining_usd"] = "128.237473951"
        _write_json(between_provider, document)
        with self.assertRaisesRegex(admission.AdmissionError, "arithmetic is inconsistent"):
            admission.verify_finalization(
                self.root,
                self.requirements_path,
                published_precommit_sha256=self.published,
    ...
    def test_finalization_rejects_absent_v13_arm_gate(self) -> None:
        self._prepare_finalization()
        (self.root / "attempt1/v13-first/evaluation/arm-gate.json").unlink()
        with self.assertRaisesRegex(admission.AdmissionError, "arm gate is missing"):
            admission._finalization_document(
                self.root,
                self.requirements_path,
                published_precommit_sha256=self.published,
                created_at=datetime(2026, 7, 16, 6, 51, 0, tzinfo=timezone.utc),
            )

    def test_finalization_rejects_failed_or_score_bearing_arm_gate(self) -> None:
        self._prepare_finalization()
        gate = self.root / "attempt1/v13-first/evaluation/arm-gate.json"
        gate.chmod(0o644)
        document = json.loads(gate.read_text(encoding="utf-8"))
        document["authorized"] = False
        document["score"] = 1.0
        _write_json(gate, document)
        gate.chmod(0o444)
        with self.assertRaisesRegex(admission.AdmissionError, "score-bearing"):
            admission._finalization_document(
                self.root,
                self.requirements_path,
                published_precommit_sha256=self.published,
    ...
    def test_finalization_rejects_swapped_arm_gates(self) -> None:
        self._prepare_finalization()
        v7 = self.root / "attempt1/v7-control/evaluation/arm-gate.json"
        v13 = self.root / "attempt1/v13-first/evaluation/arm-gate.json"
        v13.chmod(0o644)
        v13.write_bytes(v7.read_bytes())
        v13.chmod(0o444)
        with self.assertRaises(admission.AdmissionError):
            admission._finalization_document(
                self.root,
    ...
    def test_verify_arm_gate_rejects_writable_gate_and_bad_status(self) -> None:
        self._prepare_finalization()
        gate = self.root / "attempt1/v7-control/evaluation/arm-gate.json"
        gate.chmod(0o644)
        with self.assertRaisesRegex(admission.AdmissionError, "must be immutable"):
            admission.verify_arm_gate(
                self.root, self.requirements_path, variant_label="v7-control"
            )
        gate.chmod(0o444)
        status = self.root / "attempt1/v7-control/evaluation/attempt-status.json"
        status.chmod(0o644)
        document = json.loads(status.read_text(encoding="utf-8"))
        document["evaluator_status"] = "1"
        document["exit_status"] = 1
        _write_json(status, document)
        status.chmod(0o444)
        with self.assertRaisesRegex(admission.AdmissionError, "not exactly completed"):
            admission.verify_arm_gate(
                self.root, self.requirements_path, variant_label="v7-control"
            )

    def test_verify_arm_gate_rejects_truncated_proxy_stop_evidence(self) -> None:
        self._prepare_finalization()
        proxy = self.root / "attempt1/v7-control/evaluation/proxy.log"
        proxy.chmod(0o644)
        first = proxy.read_bytes().splitlines(keepends=True)[0]
        proxy.write_bytes(first)
        proxy.chmod(0o444)
        with self.assertRaisesRegex(admission.AdmissionError, "start/stop evidence"):
            admission.verify_arm_gate(
                self.root, self.requirements_path, variant_label="v7-control"
            )

    def test_verify_arm_gate_rejects_equal_cost_proxy_counter_mutation(self) -> None:
        self._prepare_finalization()
        proxy = self.root / "attempt1/v7-control/evaluation/proxy.log"
        proxy.chmod(0o644)
        ...
        proxy.chmod(0o444)
        with self.assertRaisesRegex(admission.AdmissionError, "counters do not match"):
            admission.verify_arm_gate(
                self.root, self.requirements_path, variant_label="v7-control"
            )

    def test_arm_gate_closed_schema_mutation_matrix(self) -> None:
        self._prepare_finalization()
        gate = self.root / "attempt1/v7-control/evaluation/arm-gate.json"
        original = gate.read_bytes()

        def extra_field(document: dict) -> None:
            document["unexpected"] = True

        def wrong_denominator(document: dict) -> None:
            document["denominators"]["top_20"] = 41

        def missing_validation_count(document: dict) -> None:
            document["validation_counts"].pop("empty_answers")

        def nonzero_validation(document: dict) -> None:
            document["validation"]["empty_answers"] = [{"question_id": "q00"}]
            document["validation_counts"]["empty_answers"] = 1

        def bad_model(document: dict) -> None:
            document["usage"]["request_model_counts"]["attacker/model"] = 1

        def bad_provider(document: dict) -> None:
            document["usage"]["provider_counts"]["AttackerCloud"] = 1

        def over_fuse(document: dict) -> None:
            document["usage"]["cost_usd"] = 2.51

        def score_content(document: dict) -> None:
            document["metrics"] = {"top_20": {"correct": 42}}

        for name, mutation in (
            ("extra-field", extra_field),
            ("wrong-denominator", wrong_denominator),
            ("missing-validation-count", missing_validation_count),
            ("nonzero-validation", nonzero_validation),
            ("bad-model", bad_model),
            ("bad-provider", bad_provider),
            ("over-fuse", over_fuse),
            ("score-content", score_content),
        ):
        ...
        gate.chmod(0o644)
        gate.unlink()
        hardlink_source = gate.with_name("arm-gate-hardlink-source.json")
        hardlink_source.write_bytes(original)
        hardlink_source.chmod(0o444)
        os.link(hardlink_source, gate)
        with self.assertRaisesRegex(admission.AdmissionError, "hard-linked"):
            admission.verify_arm_gate(
                self.root, self.requirements_path, variant_label="v7-control"
            )

    def test_exact_ledger_cost_above_fuse_cannot_round_into_gate(self) -> None:
        self._prepare_finalization()
        variant = self.variants["v7-control"]
        ledger = self.root / variant["ledger_path"]
        ledger.chmod(0o644)
        ...
        events[0]["cost_usd"] = 2.4000000000004
        ledger.write_text(
            "\n" + "\n".join(json.dumps(event, sort_keys=True) for event in events) + "\n",
            encoding="utf-8",
        )
        ledger.chmod(0o444)
        audit_path = self.root / "attempt1/v7-control/evaluation/ledger-audit.json"
        ...
        with self.assertRaisesRegex(admission.AdmissionError, "exact ledger cost exceeds"):
            admission._authorization_document(
                self.root,
                self.requirements_path,
                phase_name="before-v13",
                run_root=self.variants["v13-first"]["run_root"],
                ...
                dataset_path="dataset.json",
                published_precommit_sha256=self.published,
                created_at=datetime(2026, 7, 16, 6, 49, 0, tzinfo=timezone.utc),
                historical_ledger_payloads={
                    "v7-control": ledger.read_bytes(),
                    "v13-first": bytes.fromhex(
                        self.variants["v13-first"]["initial_ledger_bytes_hex"]
                    ),
                },
            )


class BundleMetadataBindingTests(unittest.TestCase):
    def test_every_sibling_path_hash_binding_matches_physical_bytes(self) -> None:
        bundle = (
            REPOSITORY
            / "benchmark_records/reproduction-v13-paid-paired-scoring-r5-20260716"
        )
        documents = [
            bundle / "paid-paired-scoring-protocol-r5.json",
            bundle / "execution-authorization-requirements.json",
            bundle / "input-bindings-r5.json",
            bundle / "preflight-r5.json",
        ]
        allowed_symlink = Path("vendor/memory-benchmarks/.venv/bin/python")
        checked: list[tuple[Path, str, str]] = []
        historical_bindings: list[tuple[Path, str, str]] = []
        frozen_bindings: dict[str, str] = {}
        for line in (bundle / "BOUND_INPUTS_SHA256SUMS").read_text(
            encoding="utf-8"
        ).splitlines():
            if not line.strip():
                continue
            digest, relative = line.split("  ", 1)
            frozen_bindings[relative] = digest

        def visit(source: Path, value: object) -> None:
            if isinstance(value, dict):
                ...
                for path_key, digest_key in pairs:
                    relative = Path(value[path_key])
                    self.assertFalse(relative.is_absolute(), (source, path_key))
                    self.assertNotIn("..", relative.parts, (source, path_key))
                    expected = value[digest_key]
                    self.assertRegex(expected, r"^[0-9a-f]{64}$")
                    candidate = REPOSITORY / relative
                    is_historical_external = not candidate.is_relative_to(bundle)
                    if candidate.is_symlink():
                        self.assertEqual(relative, allowed_symlink)
                    actual = _sha(candidate) if candidate.is_file() else None
                    if actual != expected and is_historical_external:
                        # Paths outside this immutable bundle are snapshots of
                        # live-at-freeze inputs. Product code and its tests may
                        # evolve later; the historical digest must remain bound
                        # by the bundle's sealed input manifest instead of being
                        # rewritten to match today's checkout.
                        self.assertEqual(
                            frozen_bindings.get(relative.as_posix()), expected
                        )
                        historical_bindings.append((source, path_key, digest_key))
                    else:
                        self.assertTrue(
                            candidate.is_file(), (source, path_key, relative)
                        )
                        self.assertEqual(actual, expected, (source, path_key))
                    checked.append((source, path_key, digest_key))
        ...
        for source in documents:
            visit(source, json.loads(source.read_text(encoding="utf-8")))
        self.assertEqual(len(checked), 94)
        self.assertGreaterEqual(len(historical_bindings), 5)
...
class HardenedWrapperMutationTests(unittest.TestCase):
    def test_static_closed_tree_and_guarded_runtime_controls_are_present(self) -> None:
        ...
        self.assertNotIn("| tee", wrapper)
        self.assertLess(wrapper.index("cleanup_proxy\nPROXY_PID=\"\""), wrapper.index("$ARM_GATE_GUARD"))
        self.assertIn('chmod a-w "$USAGE" "$EVALUATOR_LOG" "$PROXY_LOG" "$HEALTH"', wrapper)
