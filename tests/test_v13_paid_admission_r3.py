...
VERIFIER_PATH = (
    REPOSITORY
    / "benchmark_records/reproduction-v13-paid-paired-scoring-r3-20260716/verify_dynamic_admission.py"
)
...
class DynamicAdmissionMutationTests(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        shutil.copy2(
            REPOSITORY
            / "benchmark_records/reproduction-v13-paid-paired-scoring-r3-20260716/verify_evaluator_ledger.py",
            self.ledger_verifier,
        )
        self.telemetry_capture = self.revision / "capture_provider_telemetry.py"
        shutil.copy2(
            REPOSITORY
            / "benchmark_records/reproduction-v13-paid-paired-scoring-r3-20260716/capture_provider_telemetry.py",
            self.telemetry_capture,
        )
        self.fx_capture = self.revision / "capture_ecb_fx.py"
        shutil.copy2(
            REPOSITORY
            / "benchmark_records/reproduction-v13-paid-paired-scoring-r3-20260716/capture_ecb_fx.py",
            self.fx_capture,
        )
        self.credential_launcher = self.revision / "launch_with_openrouter_key.sh"
        shutil.copy2(
            REPOSITORY
            / "benchmark_records/reproduction-v13-paid-paired-scoring-r3-20260716/launch_with_openrouter_key.sh",
            self.credential_launcher,
        )
        self.paid_wrapper = self.revision / "run_paid_variant_hardened.sh"
        shutil.copy2(
            REPOSITORY
            / "benchmark_records/reproduction-v13-paid-paired-scoring-r3-20260716/run_paid_variant_hardened.sh",
            self.paid_wrapper,
        )
        self.harness_guard = self.revision / "run_harness_guarded.py"
        shutil.copy2(
            REPOSITORY
            / "benchmark_records/reproduction-v13-paid-paired-scoring-r3-20260716/run_harness_guarded.py",
            self.harness_guard,
        )
