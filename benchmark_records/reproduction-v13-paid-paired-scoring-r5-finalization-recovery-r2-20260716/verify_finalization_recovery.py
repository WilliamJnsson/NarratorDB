...
PROTOCOL_SCHEMA = "narratordb.v13-paid-r5-offline-finalization-recovery-protocol.v2"
...
def _validate_bound_inputs(root: Path, protocol: Mapping[str, Any]) -> None:
    ...
    if (
        ...
        raise RecoveryError("R1 terminal record binding changed")
    ...
    if prior_checksum_entries != {prior_record.name: prior["record_sha256"]}:
        raise RecoveryError("R1 terminal checksum semantics changed")
    ...
    if (
        ...
        or prior_failure.get("exit_status") != 67
        or prior_failure.get("failed_launcher_line") != 39
        or prior_failure.get("missing_path") != "/usr/bin/realpath"
        or prior_policy.get("r1_is_terminal") is not True
        or prior_policy.get("r1_overwrite_delete_resume_or_retry_allowed") is not False
        or prior_publication.get("recovery_output_root_created") is not False
        or prior_publication.get("stage_a_envelope_created") is not False
        or prior_publication.get("score_bearing_content_read_or_published") is not False
        or prior_zero_activity.get("sandbox_process_started") is not False
        or prior_zero_activity.get("worker_process_started") is not False
    ):
        raise RecoveryError("R1 terminal semantics changed")
    r1_manifest = _repository_path(root, prior["r1_seal_manifest_path"])
    r1_payload = _require_immutable(
        r1_manifest, maximum=MAX_JSON_BYTES, exact_mode=0o444
    )
    if _sha256(r1_payload) != prior["r1_seal_manifest_sha256"]:
        raise RecoveryError("R1 seal changed")
    r1_entries = _parse_manifest(r1_payload, basename_only=True)
    r1_bundle = _repository_path(root, prior["r1_bundle_root"])
    if (
        len(r1_entries) != 10
        or r1_manifest.parent != r1_bundle
        or {item.name for item in r1_bundle.iterdir()}
        != set(r1_entries) | {"SHA256SUMS"}
    ):
        raise RecoveryError("R1 closed-world bundle changed")
...
def _run_stage_b(
    ...
    *,
    ...
    if _remove_orphan_release_staging(output):
        raise RecoveryError("precommit crash staging destroyed; recovery-r2 is terminal")
    terminal_path = _repository_path(
        root, protocol["output"]["failed_status_path"], must_exist=False
    )
    if terminal_path.exists() or terminal_path.is_symlink():
        raise RecoveryError("recovery-r2 is already terminal")
