...
recovery="$root/benchmark_records/reproduction-v13-paid-paired-scoring-r5-finalization-recovery-r2-20260716"
launcher="$recovery/run_offline_recovery.sh"

if [[ "${1-}" != "--clean-internal" ]]; then
  if (( $# != 1 )) || [[ "$1" != stage-a && "$1" != stage-b && "$1" != preflight-smoke ]]; then
    exit 64
  fi
  if [[ "$1" == preflight-smoke ]]; then
    exec /usr/bin/env -i \
      /bin/bash --noprofile --norc -p "$launcher" --clean-internal "$1"
  fi
  seal="${NARRATORDB_R5_RECOVERY_R2_PRECOMMIT_SHA256-}"
  exec /usr/bin/env -i \
    NARRATORDB_R5_RECOVERY_R2_PRECOMMIT_SHA256="$seal" \
    /bin/bash --noprofile --norc -p "$launcher" --clean-internal "$1"
fi
if (( $# != 2 )) || [[ "$2" != stage-a && "$2" != stage-b && "$2" != preflight-smoke ]]; then
  exit 64
fi
stage="$2"

protocol="$recovery/recovery-protocol-r2.json"
profile="$recovery/offline-recovery.sb"
worker="$recovery/verify_finalization_recovery.py"
executable_manifest="$recovery/launcher-executables-r2.json"
output="$root/reports/longmemeval-intelligence-dev42-v13-paid-paired-scoring-r5-finalization-recovery-r2-20260716"
...
if [[ ! -f "$executable_manifest" || -L "$executable_manifest" ]] \
  || ! hash_file "$executable_manifest" 7b3fb86bcc53938fb453b3629a3c7bea111aa166b7865a36f4dbc7691868640b; then
  exit 70
fi

umask 077
scratch="$(/usr/bin/mktemp -d /private/tmp/narratordb-r5-recovery-r2.XXXXXXXX)"
cleanup() {
