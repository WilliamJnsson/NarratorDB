#!/usr/bin/env python3
"""Admit the DeepSeek route within the original five-dollar campaign envelope."""
...
PRIOR = Decimal("0.015344432")
CANARY = Decimal("0.084655568")
ARMS = Decimal("4.90")
...
def main() -> int:
    ...
    if prior.get("status") != "abandoned-before-benchmark-scoring" or prior.get("budget_audit", {}).get("conservative_total_canary_cost_or_reservation_usd") != str(PRIOR):
        raise RuntimeError("prior conservative exposure audit mismatch")
    ...
    document = {
        "schema_version": "narratordb.v17-deepseek-campaign-admission.v1",
        "created_at_utc": now.replace(microsecond=0).isoformat().replace("+00:00", "Z"),
        "admitted": True,
        "checks": checks,
        "prior_conservative_exposure_usd": str(PRIOR),
        ...
        "prior_audit_sha256": hashlib.sha256(args.prior_audit.read_bytes()).hexdigest(),
    }
