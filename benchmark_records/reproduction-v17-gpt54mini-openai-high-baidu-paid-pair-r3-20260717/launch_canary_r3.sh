...
set -euo pipefail
[[ $# -eq 1 && $1 == canary-r3 ]] || { builtin printf '%s\n' "usage: $0 canary-r3" >&2; exit 2; }
V17_SCRIPT_DIR=$(CDPATH= cd -- "$(/usr/bin/dirname -- "$0")" && pwd -P)
V17_ROOT=$(CDPATH= cd -- "$V17_SCRIPT_DIR/../.." && pwd -P)
[[ $(pwd -P) == "$V17_ROOT" ]] || exit 1
V17_PRECOMMIT="$V17_ROOT/benchmark_records/precommits/longmemeval_dev42_v17_gpt54mini_openai_high_baidu_paid_pair_r3_precommit_20260717.json"
V17_PUBLISHED=${NARRATORDB_V17_PAID_R3_PRECOMMIT_SHA256:-}
[[ $V17_PUBLISHED =~ ^[0-9a-f]{64}$ ]] || exit 1
[[ $(/usr/bin/shasum -a 256 "$V17_PRECOMMIT" | /usr/bin/awk '{print $1}') == "$V17_PUBLISHED" ]] || exit 1
...
V17_RUN="$V17_ROOT/reports/longmemeval-intelligence-dev42-v17-gpt54mini-openai-high-baidu-paid-pair-r1-20260717/canary-r3/evaluation"
/bin/mkdir -p "$V17_RUN"
V17_USAGE="$V17_RUN/openrouter-usage.jsonl"
V17_PROXY_LOG="$V17_RUN/proxy.log"
V17_HEALTH="$V17_RUN/proxy-health-before.json"
V17_RESULT="$V17_RUN/canary-result.json"
for V17_PATH in "$V17_USAGE" "$V17_PROXY_LOG" "$V17_HEALTH" "$V17_RESULT"; do
  [[ ! -e $V17_PATH && ! -L $V17_PATH ]] || exit 1
done
...
OPENROUTER_API_KEY=$V17_SECRET "$V17_PYTHON" -I -S -B \
  ...
  --public-benchmark \
  --usage-log "$V17_USAGE" \
  --max-cost-usd 0.089804068 \
  --request-reservation-usd 0.005 \
...
"$V17_PYTHON" -I -S -B "$V17_SCRIPT_DIR/route_canary_r3.py" \
